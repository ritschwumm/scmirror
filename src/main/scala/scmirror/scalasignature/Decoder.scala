	...
	*/
	...
	def parseSignature(bytes:Array[Byte]):Seq[Entry] = {
		...
		pb.toIndexedSeq.zipWithIndex map Tuples.runcurry3 collect {
			...
			case (PickleFormat.NOtpe,				body, index)	=> readNOtpe(				Ref(index), pickleBuffer(body))
			case (PickleFormat.NOPREFIXtpe,			body, index)	=> readNOPREFIXtpe(			Ref(index), pickleBuffer(body))
	...
	def readNOtpe(self:Ref, pb:PickleBuffer):NOtpe	= NOtpe(self)
	
	def readNOPREFIXtpe(self:Ref, pb:PickleBuffer):NOPREFIXtpe	= NOPREFIXtpe(self)
